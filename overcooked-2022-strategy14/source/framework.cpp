...
void get_task_list()//根据订单种类生成任务列表
{
    ...
    for(int i = 0; i < totalOrderCount; i++){
        ...
        //贪心策略，如果有需要烹饪的食材，先把食材放锅里，然后去做其他事情，最后再把锅里的食材放到盘子里
        //这个实验中，s_fish和s_rice数量和不会超过2，总食材数量不会超过3
        //由于oj的特殊判定，最终盘子里的食材是要按照字典序排列的
        //s_fish和s_rice单独出现的时候，就先把食材放到锅里，然后干别的事情，最后再把食材放到盘子里
        //s_fish和s_rice一起出现的时候，就先把鱼放到锅里，然后把米放到锅里，然后干别的事情，最后再把食材放到盘子里
        if(std::find(totalOrder[i].recipe.begin(), totalOrder[i].recipe.end(), "s_fish") != totalOrder[i].recipe.end() && std::find(totalOrder[i].recipe.begin(), totalOrder[i].recipe.end(), "s_rice") == totalOrder[i].recipe.end()){
            //只有s_fish,没有s_rice
            //先把鱼放到锅里
            task_list[totalOrder[i].recipe].push_back(Task(Move, "fish"));
            task_list[totalOrder[i].recipe].push_back(Task(Pick, "fish"));
            ...
            task_list[totalOrder[i].recipe].push_back(Task(Put, "Pan"));
            //然后去做别的事情
            for(int j = 0; j < len -1; ++j){//s_fish一定是最后一个食材
        ...
        else if(std::find(totalOrder[i].recipe.begin(), totalOrder[i].recipe.end(), "s_rice") != totalOrder[i].recipe.end() && std::find(totalOrder[i].recipe.begin(), totalOrder[i].recipe.end(), "s_fish") == totalOrder[i].recipe.end()){
            //只有s_rice,没有s_fish
            //先把米放到锅里
            task_list[totalOrder[i].recipe].push_back(Task(Move, "rice"));
            task_list[totalOrder[i].recipe].push_back(Task(Pick, "rice"));
            ...
            task_list[totalOrder[i].recipe].push_back(Task(Put, "Pot"));
            //然后去做别的事情
            for(int j = 0; j < len - 1; ++j){//s_rice一定是最后一个食材
                if(totalOrder[i].recipe[j] == "fish" || totalOrder[i].recipe[j] == "rice" || totalOrder[i].recipe[j] == "kelp"){
                    task_list[totalOrder[i].recipe].push_back(Task(Move, totalOrder[i].recipe[j]));
                    task_list[totalOrder[i].recipe].push_back(Task(Pick, totalOrder[i].recipe[j]));
                    task_list[totalOrder[i].recipe].push_back(Task(Move, "Plate"));
        ...
        else if(std::find(totalOrder[i].recipe.begin(), totalOrder[i].recipe.end(), "s_fish") != totalOrder[i].recipe.end() && std::find(totalOrder[i].recipe.begin(), totalOrder[i].recipe.end(), "s_rice") != totalOrder[i].recipe.end()){
            //s_fish和s_rice一起出现
            //先把鱼放到锅里
            task_list[totalOrder[i].recipe].push_back(Task(Move, "fish"));
            task_list[totalOrder[i].recipe].push_back(Task(Pick, "fish"));
            ...
            task_list[totalOrder[i].recipe].push_back(Task(Put, "Pot"));
            //然后去做别的事情
            for(int j = 0; j < len - 2; ++j){//s_fish和s_rice一定是最后两个食材
                if(totalOrder[i].recipe[j] == "fish" || totalOrder[i].recipe[j] == "rice" || totalOrder[i].recipe[j] == "kelp"){
                    task_list[totalOrder[i].recipe].push_back(Task(Move, totalOrder[i].recipe[j]));
                    task_list[totalOrder[i].recipe].push_back(Task(Pick, totalOrder[i].recipe[j]));
                    task_list[totalOrder[i].recipe].push_back(Task(Move, "Plate"));
...
void init()
{
    ...
    change_move_strategy[0] = change_move_strategy[1] = false;
}
