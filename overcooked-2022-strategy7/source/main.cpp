...
int main()
{
    ...
    for (int i = 0; i < totalFrame; i++)
    {
        ...
        if (skip) continue;
        update_dynamic_plate_pos();
        /* 输出当前帧的操作，此处仅作示例 */
        std::cout << "Frame " << i << "\n";
        //std::string player0_Action = "Move R";
        //std::string player1_Action = "Move U";
        std::pair<std::string, std::string> nxt_action = nxtaction();
        std::string player0_Action = nxt_action.first;
        std::string player1_Action = nxt_action.second;
