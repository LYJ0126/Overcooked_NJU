        ...
        for(int i = 0; i <entityCount; ++i){
            if(Entity[i].entity[0] == "Plate"){
                if(Entity[i].entity.size() == 1){//空盘子
                    flag = true;
                    last_plate_location = Point(Entity[i].x, Entity[i].y);
                    break;
                }
            }
        }
        //std::cerr<<"flag:"<<flag<<"\n";
        //std::cerr<<"last_plate_location:"<<last_plate_location.x<<" "<<last_plate_location.y<<"\n";
        if(flag){//有空盘子
            cooking.current_tasks = task_list[Order[0].recipe];
        }
        else{
...
Point get_cook_dst()
{
    ...
    if(cook_object == "fish" || cook_object == "rice" || cook_object == "kelp"){
        ...
        if(last_plate_location.x < 0 || last_plate_location.y < 0){
            ...
            //}
            //这种情况不会发生
            std::cerr << "Error: last_plate_location error!\n";
            assert(0);
        }
        else{
...
std::string deal_with_task(Task task, int player_id)
{
    ...
    if(cooking.task_index == cooking.current_tasks.size()) {
        ...
        if(cooking.current_tasks[cooking.task_index - 1].object == "ServiceWindow"){
