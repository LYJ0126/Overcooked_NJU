...
int change_direction(int prev, int next, int player_id)
{
    ...
    if(Players[player_id].adjust) return prev;
    else return -1;
}
...
std::pair<int, std::string> Move_along_the_route(Point src, Point dst, int player_id)
{
    ...
    if(Players[player_id].cur_direction == move_direction){
        ...
        int direction = change_direction(Players[player_id].cur_direction, move_direction, player_id);
...
/*std::pair<int, std::string> Avoid(Point src, Point dst, int player_id)
{
    Point player_src = get_player_src(src, player_id);
    if(arriving(player_src, dst, player_id)){
        Players[player_id].cur_direction = -1;
        if(arrived(src, dst, player_id)){
            return {1, "Move "};
        }
        else return {0, "Move " + slightly_move(src, dst, player_id)};
    }
    int move_direction = search_path_direction(player_id, player_src, dst);
    if(Players[player_id].cur_direction == move_direction){
        if(move_direction < 0) return {0, "Move "};
        else return {0, "Move " + directions[move_direction]};
    }
...
std::string Avoid(Point src, Point dst, int player_id)
{
    Point player_src = get_player_src(src, player_id);
    //std::cerr<<"src: "<<player_src.x<<" "<<player_src.y<<'\n';
    //std::cerr<<"dst: "<<dst.x<<" "<<dst.y<<'\n';
    if(arriving(player_src, dst, player_id)){
        Players[player_id].cur_direction = -1;
        if(arrived(src, dst, player_id)){
            return "Move ";
        }
        else return "Move " + slightly_move(src, dst, player_id);
    }
    int move_direction = search_path_direction(player_id, player_src, dst);
    if(Players[player_id].cur_direction == move_direction){
        if(move_direction < 0) return "Move ";
        else return "Move " + directions[move_direction];
    }
    else{
        int direction = change_direction(Players[player_id].cur_direction, move_direction, player_id);
        //std::cerr<<"id:"<<player_id<<" direction: "<<direction<<std::endl;
        if(direction < 0) return "Move ";
        else return "Move " + directions[direction];
    }
    //std::cerr<<"here\n";
    if(Players[player_id].cur_direction < 0) return "Move ";
    else return "Move " + directions[Players[player_id].cur_direction];
    std::cerr << "Error: Avoid" << std::endl;
    assert(0);
}