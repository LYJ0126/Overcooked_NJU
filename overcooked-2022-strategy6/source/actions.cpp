...
std::string deal_with_task(Task task, int player_id)
{
    ...
    //std::cerr<<"player0_avoid:"<<avoid_status[0]<<" player1_avoid:"<<avoid_status[1]<<'\n';
    if(avoid_status[player_id]){//玩家处于避让状态
        //std::cerr << "Avoidig "<<player_id<<" dst: "<<avoiding_pos[player_id].x<<" "<<avoiding_pos[player_id].y<<'\n';
        std::string result = Avoid(Point(Players[player_id].x, Players[player_id].y), avoiding_pos[player_id], player_id);
        //if(res.first == 1) avoid_status[player_id] = false;
        //一直等到另一个玩家到达其目的地
        //std::cerr<<player_id<<' '<<avoid_status[player_id];
        if(player_id == 0){
            if(player1_work.task_index != former_index[1]) avoid_status[player_id] = false;
        }
        else{
            if(player0_work.task_index != former_index[0]) avoid_status[player_id] = false;
        }
        return result;
    }
    switch (task.action)
...
std::pair<std::string, std::string> nxtaction()
{
    ...
    //判断卡死，如果两个玩家都卡死，则需要避让
    if(player0_work.current_tasks[player0_work.task_index].action == Move && player1_work.current_tasks[player1_work.task_index].action == Move && player0_action == "Move " && player1_action == "Move " && Players[0].cur_direction == -1 && Players[1].cur_direction == -1){
        //std::cerr<<"Both players are stuck\n";
        //counter++;
        //std::cerr<<"counter: "<<counter<<'\n';
        //if(counter >= 20){
            Point temp_pos0;
            if((int)plate_temp_pos0.x == 0) temp_pos0 = Point(plate_temp_pos0.x + 1.5, plate_temp_pos0.y + 0.5);
            else if((int)plate_temp_pos0.x == width - 1) temp_pos0 = Point(plate_temp_pos0.x - 0.5, plate_temp_pos0.y + 0.5);
            else if((int)plate_temp_pos0.y == 0) temp_pos0 = Point(plate_temp_pos0.x + 0.5, plate_temp_pos0.y + 1.5);
            else if((int)plate_temp_pos0.y == height - 1) temp_pos0 = Point(plate_temp_pos0.x + 0.5, plate_temp_pos0.y - 0.5);
            Point temp_pos1;
            if((int)plate_temp_pos1.x == 0) temp_pos1 = Point(plate_temp_pos1.x + 1.5, plate_temp_pos1.y + 0.5);
            else if((int)plate_temp_pos1.x == width - 1) temp_pos1 = Point(plate_temp_pos1.x - 0.5, plate_temp_pos1.y + 0.5);
            else if((int)plate_temp_pos1.y == 0) temp_pos1 = Point(plate_temp_pos1.x + 0.5, plate_temp_pos1.y + 1.5);
            else if((int)plate_temp_pos1.y == height - 1) temp_pos1 = Point(plate_temp_pos1.x + 0.5, plate_temp_pos1.y - 0.5);
            //优先让优先级低的玩家避让
            if(player0_orderid < player1_orderid){//玩家1优先级低
                //下一帧的时候会避让
                //寻找避让去处
                if((int)Players[1].x != (int)temp_pos1.x && (int)Players[1].y != (int)temp_pos1.y && search_path_direction(1, Point(Players[1].x, Players[1].y), temp_pos1) >= 0){
                    avoiding_pos[1] = temp_pos1;
                    avoid_status[1] = true;
                    former_index[0] = player0_work.task_index;
                }
                else if((int)Players[1].x != player_initial_pos[1].x && (int)Players[1].x != player_initial_pos[1].y && search_path_direction(1, Point(Players[1].x, Players[1].y), player_initial_pos[1]) >= 0){
                    avoiding_pos[1] = player_initial_pos[1];
                    avoid_status[1] = true;
                    former_index[0] = player0_work.task_index;
                }
                else if((int)Players[0].x != (int)temp_pos0.x && (int)Players[0].y != (int)temp_pos0.y && search_path_direction(0, Point(Players[0].x, Players[0].y), temp_pos0) >= 0){
                    avoiding_pos[0] = temp_pos0;
                    avoid_status[0] = true;
                    former_index[1] = player1_work.task_index;
                }
                else if((int)Players[0].x != (int)player_initial_pos[0].x && (int)Players[0].y !=(int)player_initial_pos[0].y && search_path_direction(0, Point(Players[0].x, Players[0].y), player_initial_pos[0]) >= 0){
                    avoiding_pos[0] = player_initial_pos[0];
                    avoid_status[0] = true;
                    former_index[1] = player1_work.task_index;
                }
                else{
                    std::cerr << "Error: Can't find avoiding position\n";
                    assert(0);
                }
            }
            else{//玩家0优先级低
                //下一帧的时候会避让
                //寻找避让去处
                if((int)Players[0].x != (int)temp_pos0.x && (int)Players[0].y != (int)temp_pos0.y && search_path_direction(0, Point(Players[0].x, Players[0].y), temp_pos0) >= 0){
                    avoiding_pos[0] = temp_pos0;
                    avoid_status[0] = true;
                    former_index[1] = player1_work.task_index;
                }
                ...
                else if((int)Players[1].x != (int)temp_pos1.x && (int)Players[1].y != (int)temp_pos1.y && search_path_direction(1, Point(Players[1].x, Players[1].y), temp_pos1) >= 0){
                    avoiding_pos[1] = temp_pos1;
                    avoid_status[1] = true;
                    former_index[0] = player0_work.task_index;
                }
