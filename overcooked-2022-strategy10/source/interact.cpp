...
std::string get_interact_direction(Point dst){
    ...
    std::cerr << "Error: get_interact_direction" << std::endl;
    assert(0);
}
...
std::pair<int, std::string> Interact_with_object(Point dst){
    ...
    for(int i = 0; i < entityCount; ++i){
        if(dst.x == Entity[i].x && dst.y == Entity[i].y){
            int sum = 1;
            if(Entity[i].entity[0] == "DirtyPlates") sum = Entity[i].sum;
            if(Entity[i].currentFrame == sum * Entity[i].totalFrame - 1) return {1, "Interact " + interact_direction};//实体已经完成交互
